...
public final class BenchmarkEngine: @unchecked Sendable {
    ...
    public func runBenchmark(
        ...
        let nativeMeasuredMBs = (try? BenchmarkDatasetGenerator.shared.measureNativeSystemZipThroughput(samplePath: sampleFilePath, targetMB: size.sizeMB)) ?? 0.0
        let nativeEstimatedSeconds = nativeMeasuredMBs > 0 ? (size.sizeMB / nativeMeasuredMBs) : 0.0
        let speedup = nativeMeasuredMBs > 0 ? max(1.0, throughput / nativeMeasuredMBs) : 1.0
    ...
    public func runCustomFileBenchmark(
        ...
        let nativeMeasuredMBs = (try? BenchmarkDatasetGenerator.shared.measureNativeSystemZipThroughput(samplePath: inputPath, targetMB: dataSizeMB)) ?? 0.0
        let nativeEstimatedSeconds = nativeMeasuredMBs > 0 ? (dataSizeMB / nativeMeasuredMBs) : 0.0
        let speedup = nativeMeasuredMBs > 0 ? max(1.0, throughput / nativeMeasuredMBs) : 1.0
